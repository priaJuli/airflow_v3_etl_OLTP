...
DROP TABLE IF EXISTS transaction_db.transaction_raw;
...
CREATE TABLE transaction_db.transaction_raw (
    ...
    `redmission_rate` Nullable(Float32) ,
    ...
    `semantic_cluster_level` Nullable(Float32) 
) ENGINE=SummingMergeTree()
ORDER BY (report_date, region, ownership, hospital_id);
